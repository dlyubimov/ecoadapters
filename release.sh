...
mvn -e  release:clean org.apache.maven.plugins:maven-release-plugin:2.1:prepare -DlocalCheckout=true -DreleaseVersion=$REL_VERSION -Dtag=tag-$REL_VERSION \
-DpushChanges=false && \
mvn org.apache.maven.plugins:maven-release-plugin:2.1:perform -Darguments="-DaltDeploymentRepository=$MVN_GITHUB_RELEASE_REPO -DR -DskipTests=true" -DlocalCheckout=true -Dmaven.test.skip=true \
-DpushChanges=false && \
