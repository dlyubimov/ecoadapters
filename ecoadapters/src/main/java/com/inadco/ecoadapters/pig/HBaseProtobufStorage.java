 ...
 * <P>
 * 
 * When we save pig output, we match each column in the hbase output
 * specification to the pig output field with the same name. If hbase output
 * specification specifies protobuf conversion, the pig output field is expected
 * to be a pig tuple. Otherwise, we support pig byte arrays (no conversion) or
 * pig chararray type (converting to byte array using utf-8 encoding).
 * <P>
 * 
 * We also expected pig field by name {@link #HKEY_ALIAS} to be used as hbase
 * key. The conversions available for that are the same as for non-protobuf
 * columns, namely, pig bytearray or pig chararray.
 * <P>
 * 
 * Finally, it is prudent to note that if a mapped pig output contains null, we
 * issue a {@link Delete} (otherwise we do a normal {@link Put}). Thus, every
 * output may result in either {@link Put} or {@link Delete} or both.
 * <P>
 * 
 * It is fatal to return a null for the {@link #HKEY_ALIAS} pig field.
 * <P>
 * 
 * Note about second optional "instance" parameter of the function: sometimes if
 * multiple invocations of this store function are used on the client in the
 * same script, the udf context parameter this function uses to send over output
 * pig schema advisory to the backend, gets mixed up. So it is strongly
 * recommended to uniquely identify each case of invocation of this function in
 * the same script by passing in the 'instance' name parameter. This seems to
 * solve some problems with multiple instances of the function use .
 * <P>
 * 
 * 
 * @author dmitriy
...
public class HBaseProtobufStorage extends StoreFunc {
    ...
    public void putNext(Tuple t) throws IOException {
        ...
        if (m_keyConvStrategy == null)
            _init(); // lazy backend init
        ...
        try {
            if (put != null)
                m_recordWriter.write(NullWritable.get(), put);
            if (delete != null)
                m_recordWriter.write(NullWritable.get(), delete);
    ...
    public void checkSchema(ResourceSchema s) throws IOException {
        ...
        udfctx.getUDFProperties(SequenceFileProtobufStorage.class).setProperty(getSchemaPropName(),
        ...
        for (int i = 0; i < m_colSpec.m_cols.length; i++) {
            ...
            if (m_pigSchema.getField(hbaseCol) == null)
                throw new IOException(String.format("Unable to map output column %s in pig output. ", hbaseCol));
        }
    ...
    private void _init() throws IOException {
        try {
            if (m_pigSchema == null) {
                ...
                UDFContext udfctx = UDFContext.getUDFContext();
                String schemaStr =
                    udfctx.getUDFProperties(SequenceFileProtobufStorage.class).getProperty(getSchemaPropName());
            ...
            for (int i = 0; i < m_colSpec.m_cols.length; i++) {
                ...
                if (pos < 0)
                    throw new IOException(String.format("column %s is not found in the pig output.", col));
                ...
                if (m_colSpec.m_msgDesc[i] != null) { // this column is a proto
                    ...
                    if (fs.type != DataType.TUPLE)
                        throw new IOException(String.format("Tuple is expected for pig output attribute %s.", col));
                    ...
                    default:
                        throw new IOException(
                            String.format("Unexpected/unsupported pig type for pig output for column %s.", col));
                    }
                }
            }
            ...
            if (m_keyPigSchemaIndex < 0)
                throw new IOException(
                    String
                        .format("Unable to find pig field by name %s which is expected to be output key.", HKEY_ALIAS));
            ...
            default:
                throw new IOException(
                    String
                        .format("Unexpected/unsupported pig type for pig output field %s which is supposed to be the table key.",
                                HKEY_ALIAS));
            }
