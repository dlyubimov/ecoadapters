...
public class SequenceFileProtobufLoader extends LoadFunc implements
        LoadMetadata {
    ...
    public SequenceFileProtobufLoader(String msgDescString) {
        ...
        try {
            ...
            // else m_msgDesc =
            // EcoUtil.inferDescriptorFromClassName(msgDescString);
    ...
    public static Descriptor parseMsgDesc(String msgDescString)
            throws Throwable {
        if (msgDescString.startsWith("hdfs://"))
            return EcoUtil.inferDescriptorFromFilesystem(msgDescString);
        else
            return EcoUtil.inferDescriptorFromClassName(msgDescString);
    }
    ...
    public Tuple getNext() throws IOException {
        ...
        try {
            ...
            Message msg = m_msgBuilder.clone()
                    .mergeFrom(bw.getBytes(), 0, bw.getLength()).buildPartial();
            return PigUtil
                    .protoMessage2PigTuple(msg, m_msgDesc, m_tupleFactory);
    ...
    private Tuple reportError(Throwable thr) {
        ...
        for (int i = 0; i < errInd; i++)
            msgTuple.append(null);
