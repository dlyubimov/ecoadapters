...
public class PigUtil {
    ...
    static public Tuple protoMessage2PigTuple(Message msg, Descriptor desc, TupleFactory tf) {
        ...
        for (FieldDescriptor fd : desc.getFields()) {
            if (fd.isRepeated())
                ...
                if (msg.hasField(fd))
                    msgTuple.append(parseNonRepeated(msg, fd, tf));
                else
                    msgTuple.append(null); // TBD: is it allowed in pig? don't
                                           // see why not
            }
        }
    ...
    static public Tuple emptyTuple(Descriptor desc, TupleFactory tf) {
        return tf.newTuple(desc.getFields().size() + 1);
    }
     ...
     * @param parseMap
     * @param builder
     * @param beginAttribute
     *            start with this attribute, inclusive
     * @param endAttribute
     *            end with this attribute, exclusive
     * @throws ExecException
     */
    static public void pigTuple2ProtoMessage(Tuple src,
                                             Tuple2ProtoMap parseMap,
                                             Message.Builder builder,
                                             int beginAttribute,
                                             int endAttribute) throws ExecException {
        if (src == null)
        ...
        for (int i = beginAttribute; i < endAttribute; i++) {
                    ...
                    for (Tuple tuple : bag) {
                        if (tuple.getAll().size() != 1)
                            throw new ExecException(
                                "Bag of a simple type expected, but incoming bag tuple does not have exactly 1 argument.");
            ...
            // otherwise assume simple value or map. Map is not really
            // supported.
            default:
    ...
    private static FieldSchema inferNonrepeatedFieldSchema(FieldDescriptor fd) throws PigException {
        switch (fd.getType()) {
        case BOOL:

            // return new FieldSchema(fd.getName(), DataType.BOOLEAN);
            // Actually, pig doesn't support boolean semantic correctly in
            // expression comparisons, nor it supports boolean constants (true
            // and false) in
            // semantic analysis. so we convert boolean values to int pig type,
            // i guess.
            // in future, we can turn correct boolean semantic conversion on if
            // it is fully supported in pig.
            // Right now, boolean type seems to be supported only as a result of
        ...
        case MESSAGE:
            return new FieldSchema(fd.getName(), generatePigSchemaFromProto(fd.getMessageType()), DataType.TUPLE);
        case STRING:
    ...
    private static FieldSchema inferRepeatedFieldSchema(FieldDescriptor fd) throws PigException {
        ...
        // bagSchema.schema=new Schema(contentSchema);
            ...
            // FieldSchema tupleSchema = new
            // FieldSchema(fd.getName(),DataType.TUPLE);
            // tupleSchema.schema=new Schema(contentSchema);
            // bagSchema.schema=new Schema (tupleSchema);

            // bagSchema.schema = SchemaUtil.newTupleSchema(fd.getName(), new
            // String[] {fd.getName()}, new Byte[] {contentSchema.type});
            bagSchema.schema = new Schema(contentSchema);
        }
    ...
    static private Object parseSimpleProtoType(FieldDescriptor fd, Object pigObject) throws ExecException {
        ...
        case BYTES:
            if (pigObject instanceof DataByteArray) {
                ...
                throw new ExecException("Unsupported conversion to byte array. ");
        case DOUBLE:
    ...
    static public final class Tuple2ProtoMap {
        ...
        private Map<Integer, Tuple2ProtoMap>  m_tupleMap = new HashMap<Integer, Tuple2ProtoMap>(); // additional
                                                                                                    // mappings
                                                                                                    // for
                                                                                                    // fields
                                                                                                    // that
        ...
        private Tuple2ProtoMap(Schema pigSchema, Descriptor desc) throws FrontendException {
            ...
            for (int i = 0; i < pigSchema.size(); i++) {
                ...
                if (ERROR_ATTR.equals(falias))
                    continue; // skip the generated stuff.
                if (fs.canonicalName != null)
                    fd = m_protoDesc.findFieldByName(fs.canonicalName);
                if (fd == null && falias != null)
                    fd = m_protoDesc.findFieldByName(falias);
                ...
                if (st == DataType.MAP)
                    throw new FrontendException(String.format("Pig maps are not yet supported in field %s",
                                                              fs.canonicalName));
                ...
                if (st == DataType.BAG) {
                    if (!fd.isRepeated())
                        throw new FrontendException(
                            String
                                .format("Unable to save pig field '%s': it's a bag type, so repeated field expected.",
                                        fs.canonicalName));
                    // should have subschema, whcih may mean we have a tuple or
                    // we may have a simple type wrapped into one-item schema.
                ...
                case MESSAGE:
                    ...
                    if (st == DataType.BAG && fs.schema != null) {
                        if (fs.schema == null || fs.schema.size() != 1)
                            throw new FrontendException("exactly one field is expected inside a bag");
                        containedSchema = fs.schema.getField(0).schema;

                    } else
                        containedSchema = fs.schema;

                    if (st == DataType.BAG || st == DataType.TUPLE)
                        m_tupleMap.put(i, new Tuple2ProtoMap(containedSchema, fd.getMessageType()));
                    else
                        throw new FrontendException(
                            String.format("Unable to save pig field '(%s:%s)': expected tuple or a bag of tuples.",
                                          fs.alias,
                                          fs.canonicalName));
                    break;
