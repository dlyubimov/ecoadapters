 ...
 * </PRE>
 * 
 * protobuf-msg-class-name is full java class name (with $ signs for inner
 * classes, not dots) for the protobuf message to use to deserialize the cell
 * value.
 * <P>
 * 
...
public class HBaseProtobufLoader extends LoadFunc implements LoadMetadata {
    ...
    public HBaseProtobufLoader(String colSpecStr) throws PigException {
        ...
        try {
            ...
            for (int i = 0; i < m_cols.length; i++)
                m_scan.addColumn(m_fams[i], m_cols[i]);
    ...
    public ResourceSchema getSchema(String location, Job job)
        ...
        for (int i = 0; i < m_pigSchema.length; i++) {
            String colName = Bytes.toString(m_fams[i]) + "::"
                    + Bytes.toString(m_cols[i]);
            ...
            if (m_reader.nextKeyValue()) {
                ...
                for (int i = 0; i < m_cols.length; ++i) {
                    ...
                    if (m_msgBuilder[i] == null)
                        ...
                        Message msg = m_msgBuilder[i].clone().mergeFrom(val)
                                .build();
                        tuple.append(PigUtil.protoMessage2PigTuple(msg,
                                m_msgDesc[i], m_tupleFactory));
                    }
