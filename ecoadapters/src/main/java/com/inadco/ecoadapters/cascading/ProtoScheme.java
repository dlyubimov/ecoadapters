 ...
 * <P/>
 * 
 * Since Cascading doesn't really support notion of nested messages schema, we
 * only roll-out (or roll-in) the first layer of the tuple only, unlike for Pig
 * and Hive.
 * <P>
 * 
 * Repeated fields are converted to an array of value types where applicable
 * (e.g. repeated fixed32 -> <code>int[]</code>); repeated messages are
 * converted to {@link Message}<code>[]</code>.
 * <P/>
 * 
...
public final class ProtoScheme extends SequenceFile {
    ...
    /**
     * Same message descriptor as with pig and hive etc. adapters. I.e. either
     * class name or hdfs reference to the descirptor file + ?msg="message". See
     * e.g. {@link com.inadco.ecoadapters.pig.SequenceFileProtobufLoader}, or
     * README, for details.
     * <P/>
     * 
     * @param msgDescString
    ...
    public void sinkPrepare(FlowProcess<JobConf> flowProcess, SinkCall<Void, OutputCollector> sinkCall)
        throws IOException {
    ...
    public boolean source(FlowProcess<JobConf> flowProcess, SourceCall<Object[], RecordReader> sourceCall)
        ...
        DynamicMessage msg =
            DynamicMessage.parseFrom(m_msgDesc, CodedInputStream.newInstance(bw.getBytes(), 0, bw.getLength()));
